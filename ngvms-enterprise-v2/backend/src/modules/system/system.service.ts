...
export class SystemService {
  static async getSystemHealth(io: any) {
    ...
    const [activeVisitors, queueBacklog] = await Promise.all([
      Visitor.countDocuments({ status: { $in: ['GATE_IN', 'MEET_IN'] } }),
      Visitor.countDocuments({ status: { $in: ['PENDING_GUARD', 'SENT_FOR_APPROVAL'] } })
    ]);
