...
export const registerVisitor: RequestHandler = async (req, res) => {
  ...
  try {
    ...
    io.emit('stats:update', { type: 'new_visitor' });
...
export const updateVisitorStatus: RequestHandler = async (req, res) => {
  ...
  try {
    ...
    io.to(`visitor_${updatedVisitor._id}`).emit('status:update', { status: updatedVisitor.status, visitor: updatedVisitor });

    if (body.status === 'SENT_FOR_APPROVAL' && hostRoomId) {
       io.to(`host_${hostRoomId}`).emit('visitor:forwarded', updatedVisitor);
    ...
    io.emit('visitor:update', updatedVisitor);
...
export const sendSecurityAlert: RequestHandler = async (req, res) => {
  ...
  try {
    ...
    const io: Server = req.app.get('socketio');
    io.to(`visitor_${visitor._id}`).emit('security:alert', { message: 'Your scheduled time has ended. Please proceed to the exit.' });
    
    if (visitor.hostId) {
       io.to(`host_${visitor.hostId._id.toString()}`).emit('security:alert', { visitorName: visitor.name, type });
       if (type === 'OVERSTAY') {
