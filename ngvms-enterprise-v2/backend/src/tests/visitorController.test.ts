...
app.put('/api/visitors/:id/status_staff', (req, res, next) => {
  req.user = { id: new mongoose.Types.ObjectId().toString(), name: 'Staff User', role: 'STAFF' };
  next();
}, updateVisitorStatus);

app.put('/api/visitors/:id/status_admin', (req, res, next) => {
  req.user = { id: new mongoose.Types.ObjectId().toString(), name: 'Admin User', role: 'ADMIN' };
  next();
}, updateVisitorStatus);
